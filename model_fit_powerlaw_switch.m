...
data = zeros(6, 11, 8); %AUT1
%data = zeros(6, 10, 8); %MBC2

% Loop over each sheet and read the data, only use the top 8 speakers
for sheetIdx = 1:8
...
%scatter(1:M, flip(column_means), 80, 'red','*');
plot(1:M, flip(column_means), 'r-')
%scatter(1:M, flip(model_column_means), 80, darkGreen,'*');
plot(1:M, flip(model_column_means), '--', 'Color', darkGreen)
hold off
box on
